    ...
    if (SF_GLAD_WGL_ARB_pixel_format)
    {
        ...
        if (isValid && (nbFormats > 0))
        {
            ...
            for (UINT i = 0; i < nbFormats; ++i)
            {
                ...
                const std::array   attributes = {WGL_RED_BITS_ARB,
                                                 WGL_GREEN_BITS_ARB,
                                                 WGL_BLUE_BITS_ARB,
                                                 WGL_ALPHA_BITS_ARB,
                                                 WGL_DEPTH_BITS_ARB,
                ...
                {
                    const std::array sampleAttributes = {WGL_SAMPLE_BUFFERS_ARB, WGL_SAMPLES_ARB};
                ...
                if (pbuffer)
                {
                    const std::array pbufferAttributes = {WGL_DRAW_TO_PBUFFER_ARB};
...
void WglContext::updateSettingsFromPixelFormat()
{
    ...
    if (SF_GLAD_WGL_ARB_pixel_format)
    {
        const std::array   attributes = {WGL_DEPTH_BITS_ARB, WGL_STENCIL_BITS_ARB};
